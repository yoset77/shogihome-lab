...
- **複数定跡表示**: renderer の `BookStore` は開いている定跡ごとの `BookSessionStore` コレクションと、定跡未開放時の操作・検索を受け持つデフォルトセッションを管理します。操作対象は常にアクティブなセッション（未開放時はデフォルトセッション）です。未保存のデフォルトセッションがある間は別の定跡を開けず、保存すると通常の外部セッションへ移行します。PC の棋譜ペインでは元のレイアウト（一覧＋下部コントロール行）を維持したまま、複数開いた場合は一覧領域を等分の列に分割し、必要に応じて横スクロールできる列として各セッションを表示します（列は選択可能なパス表示と閉じるボタンのみ持ち、編集・保存・追加などの操作は下部コントロール行がアクティブなセッションに対して実行）。モバイルは同じコレクションのアクティブな1冊だけを既存の定跡タブへ表示します。ダイアログを伴う編集・削除・インポートは開始時のセッションを操作対象として保持します。局面変更時は開いている全セッションを再検索し、同一パスを再度開いた場合は重複セッションを作らず既存列を選択します。同時に同じパスを開く要求も1つの処理を共有します。保存先として別の開いている定跡と同じパスは指定できません。列を閉じると対応するサーバーセッションを解放し、未保存の変更があれば破棄確認を表示します。終了に失敗した場合は再試行できるよう列を維持します。初期化では開いている定跡の形式を維持します。
