...
- **永続化アーキテクチャ**: Node.js v22+ の `node:sqlite` (SQLite 3) を使用。`data/analysis.db` に保存されます。
- **データ構造 (3テーブル構成)**:
    - `positions`: 正規化SFENと64bit Zobristハッシュ（`BigInt`）を保持。
    - `engines`: エンジン識別キー（パスや名前のハッシュ）と表示名を保持。
    - `analysis_results`: 局面・エンジン・MultiPVを主キーとした解析結果（評価値、深さ、読み筋等）。
- **正規化SFEN**: 局面の同一性を判定するため、SFEN末尾の手数（move count）を削除した形式を使用します。これにより、同じ局面であれば手順が異なっても過去の解析結果がヒットします。
- **最適化された表示順**: DBタブでは以下の優先順位でソートされます。
    1. `multipv` (昇順): 各エンジンの「最善手」を最上部に集約。
    2. `depth` (降順): 最も信頼性の高い（深く読まれた）結果を優先。
    3. `score_mate` の有無 (降順): 同じ深さなら詰み発見を優先。
    4. `score_cp` (降順): 同じ深さ・詰みなしなら評価値が高い方を優先。
- **フリッカーフリーなUX**: フロントエンドでは局面変更時に即座に「検索中」状態に移行し、APIレスポンスを待つ間の「データなし」表示によるチラつきを防止しています。
