...
| `src/node/` | **Node 実行環境共有ユーティリティ**。server、command、旧 background から共有されるログ、実行環境パス、汎用ファイル操作を保持します。ブラウザー向け renderer/common からは参照しません。 |
...
- **意図的な終了**: クライアントから `stop_engine` コマンドが送出された後の切断は「意図的な終了」とみなされ、サーバーは即座にエンジンプロセスを終了しリソースを解放します。`LanEngine.disconnect()` は、ソケットを閉じる直前にバッファ内の `stop_engine` を確実に送出する仕組みを備えています。
...
- **ファイル探索**: 指定されたディレクトリを再帰的にスキャンし、棋譜（`.kif`, `.kifu`, `.ki2`, `.ki2u`, `.csa`, `.jkf`）や定跡（`.db`, `.bin`）、局面集（`.sfen`）を抽出します。**棋譜リストはインメモリキャッシュにより、2回目以降の取得を高速化しています。**
