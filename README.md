...
- **検討結果データベース:** 検討時の評価値や読み筋をサーバー側（SQLite）に自動蓄積します。検討結果は、別の棋譜やデバイスでその局面を表示した際にも即座に参照できます。
...
- **サーバー側棋譜管理:** サーバー上の特定のディレクトリにある棋譜を、スマホやタブレットから直接読み書きできます。
- **定跡管理:** サーバー上の定跡ファイル（.db, .bin）をブラウザから利用できます。指し手の追加・編集・評価値の更新などの編集操作も可能です。
