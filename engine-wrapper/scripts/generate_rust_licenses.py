...
Usage (run from the crate root whose Cargo.lock should be audited):
    python ../scripts/generate_rust_licenses.py \
        --config ../about.toml --append licenses/RUST-THIRD-PARTY-NOTICES.txt
"""
...
def main() -> int:
    ...
    try:
        subprocess.run(
            ["cargo", "about", "generate", "--config", args.config, "--format", "json", "-o", tmp_path],
